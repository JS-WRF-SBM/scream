...
! pbuf fields for heterogeneous freezing
   ...
   logical            :: micro_tune_autoconversion = .false. ! Use Autoconversion coeff. tunning
...
subroutine micro_p3_readnl(nlfile)
  ...
  namelist /micro_nl/ &
       ...
       micro_tend_output, micro_tune_autoconversion
  ...
  if (masterproc) then
     ...
     write(iulog,'(A30,1x,L)')    'micro_tune_autoconversion: ', micro_tune_autoconversion
  ...
  call mpibcast(micro_tune_autoconversion, 1,                          mpilog,  0, mpicom)
...
!    call addfld('UMS', (/ 'lev' /), 'A',   'm/s', 'Mass-weighted snow fallspeed'               )
  ...
  subroutine micro_p3_tend(state, ptend, dtime, pbuf)
    ...
    logical :: log_qcAuto_LR           !tune coefficients for Autoconversion
    ...
    ! HANDLE AUTOCONVERSION TUNNING
    !==============
    log_qcAuto_LR = micro_tune_autoconversion
...
! thus when considering any dp/dz calculation we must also change the sign.
    ...
    call p3_main( &
         ...
         log_qcAuto_LR,               & ! IN    .true.= using CMDV-P3 autoconversion coeff.
